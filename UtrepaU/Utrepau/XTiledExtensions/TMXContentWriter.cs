...
namespace FuncWorks.XNA.XTiled {
    ...
    public class TMXContentWriter : ContentTypeWriter<Map> {
        protected override void Write(ContentWriter output, Map value) {
            ...
            output.Write(value.ImageLayers.Count);
            foreach (var il in value.ImageLayers) {
                output.Write(il.Name == null ? String.Empty : il.Name);
                output.Write(il.Opacity);
                output.Write(il.OpacityColor.A);
                output.Write(il.Visible);

                output.Write(il.Color.HasValue);
                if (il.Color.HasValue)
                {
                    output.Write(il.Color.Value.R);
                    output.Write(il.Color.Value.G);
                    output.Write(il.Color.Value.B);
                    output.Write(il.Color.Value.A);
                }

                output.Write(il.MapImages.Length);
                foreach (var i in il.MapImages) {
                    output.Write(i.ImageFileName == null ? String.Empty : i.ImageFileName);
                    output.Write(i.ImageHeight);
                    output.Write(i.ImageWidth);
                    output.Write(i.Name == null ? String.Empty : i.Name);

                    output.Write(i.ImageTransparentColor.HasValue);
                    if (i.ImageTransparentColor.HasValue)
                        output.Write(i.ImageTransparentColor.Value.A);

                    output.Write(i.Properties.Count);
                    foreach (var kv in i.Properties)
                    {
                        output.Write(kv.Key == null ? String.Empty : kv.Key);
                        output.Write(kv.Value.Value == null ? String.Empty : kv.Value.Value);
                    }
                ...
                output.Write(il.Properties.Count);
                foreach (var kv in il.Properties)
                {
                    output.Write(kv.Key == null ? String.Empty : kv.Key);
                    output.Write(kv.Value.Value == null ? String.Empty : kv.Value.Value);
                }
